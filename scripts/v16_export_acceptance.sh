...
echo "== V1.6 export API runtime =="
if ! api_health; then
  PATH="$PG_BIN:$PATH" "$ROOT/scripts/run_reader_api.sh" >/tmp/sentence-reader-v16-api.log 2>&1 &
  API_PID="$!"
...
api_health
PATH="$PG_BIN:$PATH" "$PYTHON" "$ROOT/scripts/reader_api_export_smoke.py"

echo "V1.6 export acceptance PASS"
