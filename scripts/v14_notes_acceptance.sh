...
echo "== V1.4 V1.3 data gate =="
PATH="$PG_BIN:$PATH" "$ROOT/scripts/v13_swift_data_acceptance.sh"

echo "== V1.4 static notes UI contract =="
python3 "$ROOT/scripts/check_native_reader.py"

echo "== V1.4 compile/package =="
swiftc "$ROOT/Probe/NativeSentenceReader/SentenceReaderNative.swift" \
  -o /tmp/SentenceReaderNativeV14Check \
  -framework Cocoa \
  -framework WebKit \
...
echo "== V1.4 notes API loop =="
if ! api_health; then
  PATH="$PG_BIN:$PATH" "$ROOT/scripts/run_reader_api.sh" >/tmp/sentence-reader-v14-api.log 2>&1 &
  API_PID="$!"
...
api_health
PATH="$PG_BIN:$PATH" "$PYTHON" "$ROOT/scripts/reader_api_notes_loop_smoke.py"

echo "V1.4 notes acceptance PASS"
