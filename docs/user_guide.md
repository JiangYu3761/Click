...
| --- | --- | --- |
| 添加文字备注 | `N` | 聚焦句子后打开备注 |
| 添加语音备注 | `V` | 聚焦句子后开始语音备注 |
| 整句标红 | `R` | 聚焦句子后标红 / 取消标红 |
| 关闭弹窗 | `Esc` | 关闭备注、查词、抽屉或取消聚焦 |
| 翻页 | 方向键 / `PageUp` / `PageDown` | 上一页 / 下一页 |

这些快捷键已经在 Web 阅读器层实现，会被未来 Windows 浏览器版和 Windows 桌面壳复用。它们不是系统全局快捷键，只在阅读正文聚焦后生效。
...
- Mac 端可以在备注弹窗或运行环境页选择 `FunASR` / `苹果`。
- 默认使用 Click 软件内本地语音识别，也就是 FunASR 路径。
- 苹果系统语音识别是备选：当软件层本地识别不可用时才降级使用。
- iPad / 浏览器端默认先走浏览器录音上传到 Reader API，再降级到音频文件上传、浏览器/系统识别或手动备注。
- 不调用付费语音服务。
